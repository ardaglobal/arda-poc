...
if [ ! -f "$HOME_DIR/config/genesis.json" ]; then
  ...
  arda-pocd add-genesis-account $(arda-pocd --home "$HOME_DIR" keys show "$MONIKER" -a --keyring-backend test) 1000000000000uarda --home "$HOME_DIR"
  arda-pocd gentx "$MONIKER" 900000000000uarda --chain-id "$CHAIN_ID" --home "$HOME_DIR" --keyring-backend test
